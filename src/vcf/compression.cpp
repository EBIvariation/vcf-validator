...
namespace ebi
{
  namespace vcf
  {
    ...
    void check_uncompressed_stream(std::istream & input, std::string file_name)
    {
    ...
    void check_uncompressed_stream(std::istream & input)
    {
        std::vector<char> line;
        ebi::vcf::get_magic_num(input, line);
        std::string file_extension = ebi::vcf::get_compression("", line);
        if (file_extension != NO_EXT) {
            throw std::invalid_argument{"Cannot continue with compressed file"};
        }
    }
